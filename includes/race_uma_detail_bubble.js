const columns = {
    ...
    blinker_shiyo_first_flag: "初ブリンカー(競走馬に初めてブリンカー着用)の場合は「はい」、該当しない場合はnull。取消・除外等の確定着順なしレースはカウントせず、確定着順1以上の実走のみを着用回数に含める。最新 schedule_date のレースは確定着順が null（未走）でも判定する",
    ...
    kishu_tennori_flag: "テン乗り(騎手がこの馬に初めて騎乗)の場合は「はい」、該当しない場合はnull。取消・除外等の確定着順なしレースはカウントせず、確定着順1以上の実走のみを騎乗回数に含める。最新 schedule_date のレースは確定着順が null（未走）でも判定する",
    ...
    chokyo_den_awase_uma_this_week_race_code_uma_jvd_sokuhou: "出馬表出走馬データ 調教併せ馬 今週出走する場合のrace_code_uma_jvd（速報：kol_den2ベース、配信時点の出走馬）今週出走しない場合はnull",
    chokyo_den_awase_uma_this_week_race_code_uma_jvd_kakutei: "出馬表出走馬データ 調教併せ馬 今週出走する場合のrace_code_uma_jvd（確定：kol_sei2ベース、レース結果確定済み）今週出走しない場合はnull",
    chokyo_den_awase_uma_this_week_flag_sokuhou: "出馬表出走馬データ 調教併せ馬 今週出走（速報）する場合は「はい」 該当しない場合はnull",
    chokyo_den_awase_uma_this_week_flag_kakutei: "出馬表出走馬データ 調教併せ馬 今週出走（確定）した場合は「はい」 該当しない場合はnull",
