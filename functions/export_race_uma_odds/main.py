...
STATE_TABLE_NAME = "race_uma_odds_export_state"
...
def export_race_uma_odds(request):
    """更新されたレース単勝オッズ情報をFTPにエクスポートするHTTP Cloud Function"""
    try:
        # 1. クライアントの初期化
        ...
        query = f"""
            WITH CurrentOdds AS (
                SELECT
                    *
                FROM `{PROJECT_ID}.{DATASET_ID}.race_uma_odds`
            ),
            State AS (
                SELECT
                    race_code_uma_kol,
            ...
            FROM CurrentOdds c
            LEFT JOIN State s ON c.race_code_uma_kol = s.race_code_uma_kol
            ORDER BY c.hasso_date_utc
        """
        ...
        fieldnames = [
            ...
            "hasso_date",
            "hasso_date_utc",
            "sokuhou_tansho_ninkijun_num",
            "sokuhou_tansho_odds_float",
            "sokuhou_happyo_tsukihi_jifun",
            "sokuhou_modified",
            "sokuhou_created",
            "modified",
            "created"
        ...
        for row in rows:
            ...
            hash_data = {
                ...
                "hasso_date_utc": row_data["hasso_date_utc"],
                "sokuhou_tansho_ninkijun_num": row_data["sokuhou_tansho_ninkijun_num"],
                "sokuhou_tansho_odds_float": row_data["sokuhou_tansho_odds_float"],
                "sokuhou_happyo_tsukihi_jifun": row_data["sokuhou_happyo_tsukihi_jifun"]
            }
        ...
        try:
            with ftplib.FTP(FTP_HOST) as ftp:
                ...
                for i, chunk in enumerate(chunks):
                    # チャンクごとの日付範囲を取得（ファイル名用）
                    chunk_dates = []
                    for u in chunk:
                         # hasso_date_utc or hasso_date からYYYYMMDDを取得
                         # row_data["hasso_date"] は datetime オブジェクトのはず
                         dt = u["hasso_date"]
                         if isinstance(dt, str):
                            # 万が一文字列の場合
                             try:
                                 dt = datetime.datetime.strptime(dt, '%Y-%m-%dT%H:%M:%S') # BigQuery default format often matches checks
                             except ValueError:
                                 # Fallback logic if needed, but bigquery lib usually returns datetime
                                 pass
                         ...
                         if isinstance(dt, datetime.datetime):
                             ...
                             chunk_dates.append(str(dt)[:10].replace("-", ""))

                    chunk_min_date = min(chunk_dates) if chunk_dates else "unknown"
                    chunk_max_date = max(chunk_dates) if chunk_dates else "unknown"
                    ...
                    if BUBBLE_API_URL and BUBBLE_API_KEY_SECRET_ID:
                        try:
                            ...
                            # Bubble API側で期待されるパラメータ名があれば合わせるが、指定がないので現状維持(csv_url)
                            payload = {
                                "csv_url": csv_url,
                                "table_name": table_name # 念のためテーブル名も送る
                            }
        ...
        if state_updates:
            ...
            temp_table_id = f"{PROJECT_ID}.{DATASET_ID}.temp_race_uma_odds_state_updates"
